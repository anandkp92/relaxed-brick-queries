queries = [
"""SELECT * WHERE { ?s ?p ?o }""",
"SELECT ?point ?point_type WHERE { ?point rdf:type brick:Point . ?point rdf:type ?point_type . }",
 "SELECT ?meter WHERE { ?meter rdf:type brick:Green_Button_Meter }",
""" SELECT ?t WHERE { ?t rdf:type brick:Weather_Temperature_Sensor }""",
"SELECT ?sensor WHERE { ?sensor rdf:type brick:Zone_Air_Temperature_Sensor . ?sensor brick:isPointOf ?equip }",
"SELECT ?sp WHERE { ?sp rdf:type brick:Zone_Air_Temperature_Setpoint . ?sp brick:isPointOf ?equip }",
